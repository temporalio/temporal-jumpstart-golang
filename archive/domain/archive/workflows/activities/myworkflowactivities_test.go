...
func (s *MyWorkflowActivitiesTestSuite) Test_QueryActivity() {
	s.env.RegisterActivity(TypeArchiveActivities.QueryActivity)

	args := &queries.QueryActivityRequest{ID: testhelper.RandomString()}
	val, err := s.env.ExecuteActivity(TypeArchiveActivities.QueryActivity, args)
	s.NoError(err)
...
func (s *MyWorkflowActivitiesTestSuite) Test_MutateActivity() {
	s.env.RegisterActivity(TypeArchiveActivities.MutateActivity)

	args := &commands.MutateActivityRequest{ID: testhelper.RandomString()}
	_, err := s.env.ExecuteActivity(TypeArchiveActivities.MutateActivity, args)
	s.NoError(err)
}
